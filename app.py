...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2[['App','Category','Reviews','Rating']].groupby(['Category','App']).agg({'Reviews' : 'sum','Rating' : 'mean'}).sort_values(by='Reviews',ascending=False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    ax.set_title('Installs per Top 5 Category',loc ='center', )
